...
function App(): React.JSX.Element {
  ...
  const addTestAlarm = async () => {
    try {
      ...
      const updatedAlarms = await AlarmService.getInstance().getAllAlarms();
      setAlarms(updatedAlarms);
      updateNextAlarm();

      Alert.alert('Success', 'Test alarm set for 1 minute from now');
    } catch (error) {
  ...
  const updateNextAlarm = () => {
    const next = AlarmService.getInstance().getNextAlarm();
    setNextAlarm(next);
  };
        ...
        <ScrollView style={styles.content}>
          ...
          {!dataLoading && location && (
            <>
              ...
              <TouchableOpacity
                style={[styles.speakButton, {backgroundColor: '#34C759'}]}
                onPress={addTestAlarm}>
                <Text style={styles.speakButtonText}>Add Test Alarm</Text>
              </TouchableOpacity>

              {nextAlarm && (
                <View style={[styles.card, {backgroundColor: isDarkMode ? '#1a1a1a' : '#f5f5f5'}]}>
                  <Text style={[styles.cardTitle, textStyle]}>Next Alarm</Text>
                  <Text style={[styles.nextAlarmText, textStyle]}>
                    {nextAlarm.time.toLocaleTimeString('en-US', {
                      hour: 'numeric',
                      minute: '2-digit',
                    })}
                  </Text>
